...
void init_stm0(void)
{
    ...
    IfxCpu_disableInterrupts(); /*중요한 setting 전 interrupt 발생 금지 시키기*/

    stmSfr = &MODULE_STM0;
    IfxStm_initCompareConfig(&stmConfig);

    stmConfig.ticks = STM_TIME_1MS; /*Compare 값 설정 → 왜 100000개 tick이 1ms가 될까?*/

    stmConfig.triggerPriority = IFX_INTPRIO_STM0_SR0; /*인터럽트 우선순위 설정*/
    stmConfig.typeOfService = IfxSrc_Tos_cpu0;

    IfxStm_initCompare(stmSfr, &stmConfig);

    IfxCpu_enableInterrupts(); /*중요한 setting 종료 후 interrupt 발생 허가 시키기*/
}
...
void core0_task_10ms(void)
{
    //IfxPort_setPinState(&MODULE_P10, 1, IfxPort_State_toggled);
    ;
